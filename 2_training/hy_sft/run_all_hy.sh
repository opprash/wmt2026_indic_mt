#!/bin/bash
# Master training script -- Hunyuan-MT-7B full-parameter SFT
...
#
# Recommended launch:
#   nohup bash /base/rd1/hy_sft/run_all_hy.sh > /base/rd1/train_logs_hy/run_all_hy.log 2>&1 &

# NOTE: do NOT enable `set -e` -- let the loop continue past per-task failures.
set -u

HY_DIR=/base/rd1/hy_sft

LANGS=("karbi" "targin" "kokborok" "nagamese" "bodo")
LR="3e-5"

mkdir -p /base/rd1/train_logs_hy

ts() { date '+%Y-%m-%d %H:%M:%S'; }

echo "[$(ts)] ===== Hunyuan-MT-7B full-SFT pipeline started ====="
...
echo "[$(ts)] ===== All Hunyuan-MT-7B full-SFT tasks completed ====="
